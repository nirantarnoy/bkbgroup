...
if ($id) {
    $query = "SELECT * FROM member_account WHERE member_id='$id' AND cash_in > 0 ORDER BY trans_date DESC ";
...
//    }
    foreach ($result as $row) {
       $html.='<tr>';
        ...
        $html.='<td>'.number_format($row['cash_in']).'</td>';
        if($is_all){
            $html.='<td style="text-align: center"><div onclick="deleteTrans($(this),1)" data-id="'.$row['id'].'" class="btn btn-danger btn-sm">Delete</div></td>';
        }else{
